function sigs = flica_posthoc_correlations_unc(outdir, des)

oldwarn = warning('query');
warning off stats:glmfit:IllConditioned
warning off stats:glmfit:IterationLimit

H = read_vest([outdir '/subjectCoursesFractions.txt'])';
...
le            plot(x, y, des.(fn).style)
        ...
        if stats.p(2) < 0.05
           % if stats.p(2) < 0.05/length(fs)/length(L) %take out the %s for 69,73-76 for corrected
                style = {'r-','linewidth',2};
                isSig(i,1) = stats.p(2);
    ...
    %    [beta,~,stats] = glmfit([x(:) x(:).^2], y(:));         %use this
    %    one to also include the squared term
     %   corrString{i,2} = sprintf('p=%.2g', stats.p(3));
     %   if stats.p(3) < 0.05
           % if stats.p(3) < 0.05/length(fs)/length(L) %take out %s for corrected
    %            style = {'r-','linewidth',2};
    %            isSig(i,2) = stats.p(3);
     ...
     %       plot(ans, ans*beta(2)+beta(1)+ans.^2*beta(3), style{:})
      %  end
        ...
        %% Save image output ___________________NOTE: I had to change this code because convert didnt work. commented out 100-101 and 108, added 106-107
       % fprintf(' EPS...')
      %  print('-depsc',outfile);
        ...
        fprintf(' PNG...')
        outfile_png = [outdir '/correlation_' num2str(i-1) '_' fn '.png']
       print_png(outfile_png);        
